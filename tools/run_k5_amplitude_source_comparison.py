#!/usr/bin/env python3
"""K5-B.2 compare bounded metric-amplitude source candidates.

No correction is integrated.

K5 direction is already validated by K5-A and is additionally required to align
with the SHARP fine gradient on training support. B.2 compares *scalar
magnitudes*, avoiding the phase/sign pathology of the B.1 band-pass vector.

Dimensionless source:
    detail_consensus

Metric target candidates:
A_sharp_fine_magnitude
    |grad G_fine(q)|

B_magnitude_excess
    max(|grad G_fine(q)| - |grad G_coarse(q)|, 0)

B_energy_excess
    sqrt(max(|grad G_fine(q)|^2 - |grad G_coarse(q)|^2, 0))

B_bandpass_magnitude
    |grad G_fine(q) - grad G_coarse(q)|

Each target gets the same spatial cross-fit. Wire pixels are excluded from fit.
"""
...
def main():
    ...
    band_x, band_y = fx - cx, fy - cy
    band_mag = np.hypot(band_x, band_y)

    targets = {
        "A_sharp_fine_magnitude": fine_mag,
        "B_magnitude_excess": np.maximum(fine_mag - coarse_mag, 0.0),
        "B_energy_excess": np.sqrt(
            np.maximum(fine_mag*fine_mag - coarse_mag*coarse_mag, 0.0)
        ),
        "B_bandpass_magnitude": band_mag,
    }
    ...
    finite = (
        np.isfinite(q) & (q > 0)
        & np.isfinite(detail) & np.isfinite(confidence)
    ...
    excluded = (
        ...
        if args.wire_exclusion_px > 0 else wire.copy()
    )

    border = np.ones(common.shape, bool)
    b = args.border_px
    if b:
        border[:b] = border[-b:] = False
        border[:, :b] = border[:, -b:] = False
    ...
    if int(base.sum()) < 100:
        raise RuntimeError(f"Only {int(base.sum())} direction-aligned base pixels")
    ...
    for name, target in targets.items():
        ...
        target_norm = max(
            float(np.percentile(target_selected, 90)),
            np.finfo(float).tiny,
        )
        ...
        median_scale = float(np.median(scale_arr))
        scale_spread = float(
            np.ptp(scale_arr) / max(median_scale, np.finfo(float).tiny)
        )
        ...
        candidate_reports[name] = {
            "target_units": "1/metre/native-pixel",
            "target_on_selected": stats(target[selected]),
            "source_detail_consensus_on_selected": stats(detail[selected]),
            "selected_spearman": rank_corr(detail[selected], target[selected]),
            "folds": folds,
            "fold_scales": scales,
            "scale_relative_spread": scale_spread,
            "pooled_normalized_abs_residual": stats(np.asarray(pooled)),
            "pooled_predicted_over_target_ratio": stats(np.asarray(ratios)),
            "full_scale": float(full_fit.scale),
            "wire_predicted_metric_magnitude": stats(
                full_fit.scale * detail[wire]
            ),
        }

    output.mkdir(parents=True, exist_ok=False)
    np.save(output / "selected_support_mask.npy", selected)
    np.save(output / "direction_cosine_to_sharp_fine.npy", direction_cosine)

    summary = {
        "purpose": "K5-B.2 scalar metric-amplitude source comparison",
        ...
        "support": {
            "base_direction_aligned_pixels": int(base.sum()),
            "selected_pixels": int(selected.sum()),
            "spatial_blocks": int(len(blocks)),
            "wire_pixels": int(wire.sum()),
            "selected_direction_cosine": stats(direction_cosine[selected]),
        },
        "candidates": candidate_reports,
        "guardrails": [
            "No vector band-pass sign is used for amplitude fitting.",
            "K5 direction must align with SHARP fine gradient on fitting support.",
            "Wire pixels are excluded from fitting.",
            "Wire values are extrapolated diagnostics only.",
    ...
    print("K5-B.2 amplitude-source comparison complete")
